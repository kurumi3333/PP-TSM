...
from predict.action_detect.utils.config_utils import parse_config, print_configs
from predict.action_detect.utils.preprocess import get_images

sys.path.append('../predict/action_detect')
import models.audio_infer as audio_model
import models.pptsm_infer as image_model

from predict.action_detect import logger

logger = logger.Logger()


def load_model(cfg_file="configs/config.yaml"):
...
def video_classify(video_name):
    ...
    logger.info("{} {} {}".format(np_image_features.shape, np_audio_features.shape, np_pcm_features.shape))
    logger.info("step1: feature extract time: {} min".format((t1 - t0) * 1.0 / 60))
    video_features = {
        'image_features': np_image_features,
        'audio_features': np_audio_features,
        'pcm_features': np_pcm_features
    }
...
if __name__ == '__main__':
    ...
    if not os.path.exists(dataset_dir + '/features'):
        os.makedirs(dataset_dir + '/features')
