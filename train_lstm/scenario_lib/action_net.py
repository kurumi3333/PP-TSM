...
class ActionNet:
    ...
    def build_model(self):
        ...
        lstm_forward, _ = fluid.layers.dynamic_lstm(input=lstm_forward_fc, size=self.lstm_size_img * 4,
                                                    is_reverse=False,
                                                    use_peepholes=True)
        ...
        lstm_backward_fc = fluid.layers.fc(
            ...
            bias_attr=ParamAttr(
                regularizer=fluid.regularizer.L2Decay(0.0),
                initializer=fluid.initializer.NormalInitializer(scale=0.0)))
        lstm_backward, _ = fluid.layers.dynamic_lstm(input=lstm_backward_fc, size=self.lstm_size_img * 4,
                                                     is_reverse=True,
                                                     use_peepholes=True)
